...
const COMMON_PROPERTIES = {
    ...
    projectPath: { type: "string", minLength: 1, maxLength: 4096, description: "현재 프로젝트의 절대 경로" }
};
...
function createMcpServer(bridge) {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: Object.keys(METHODS).map((name) => ({
        name,
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        try {
            ...
            const result = await bridge[METHODS[request.params.name]](request.params.arguments ?? {});
